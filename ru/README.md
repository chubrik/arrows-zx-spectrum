...
стрелочек. Проект написан для новой версии Стрелочек, демо которой уже вышло в Steam. Добавьте
Стрелочки в [список желаемого](https://store.steampowered.com/app/4210250/Logic_Arrows/), чтобы не
пропустить полный релиз.
...
- **Упаковка.** Сборочный конвейер сжимает весь процессор в один командный блок объёмом около
  6 000 символов (24 КБ): esbuild → собственный инлайнер функций → три прохода Terser →
  преобразование в стрелочные функции → самораспаковывающаяся строка.
...
Требуется Node.js 22.18 или новее. Сборка автоматически скачивает оригинальный ROM и файлы
